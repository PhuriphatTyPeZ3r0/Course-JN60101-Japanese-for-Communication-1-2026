# JN60101 Japanese for Communication 1 (ภาษาญี่ปุ่นเพื่อการสื่อสาร 1)

<div align="center">

[![Institution: PIM](https://img.shields.io/badge/Institution-PIM-003366?style=for-the-badge&logo=google-classroom&logoColor=white)](https://www.pim.ac.th/)
[![Program: CAI](https://img.shields.io/badge/Program-CAI-blue?style=for-the-badge)](https://www.pim.ac.th/)
[![Academic Year](https://img.shields.io/badge/Academic%20Year-1%2F2569%20(2026)-orange?style=for-the-badge)](https://github.com/PhuriphatTyPeZ3r0)
[![Grade](https://img.shields.io/badge/Status-In%20Progress%20(กำลังศึกษา)-yellow?style=for-the-badge)](https://github.com/PhuriphatTyPeZ3r0)
[![Obsidian Compatible](https://img.shields.io/badge/Obsidian-Vault%20Ready-7C3AED?style=for-the-badge&logo=obsidian&logoColor=white)](https://obsidian.md/)

**คลังสรุปเนื้อหา คำศัพท์ ไวยากรณ์ สคริปต์บทสนทนา และแนวข้อสอบประจำรายวิชา**  
*กลุ่มวิชาภาษาเพื่อการสื่อสาร สำนักการศึกษาทั่วไป — สถาบันการจัดการปัญญาภิวัฒน์ (PIM)*

</div>

---

## <img src="https://api.iconify.design/material-symbols:list-alt-outline.svg?color=%236366F1" width="20" height="20" align="center" /> สารบัญ (Table of Contents)
- [<img src="https://api.iconify.design/material-symbols:menu-book-outline.svg?color=%230284C7" width="16" height="16" align="center" /> 1. ข้อมูลรายวิชาเบื้องต้น (Course Information)](#-1-ข้อมูลรายวิชาเบื้องต้น-course-information)
- [<img src="https://api.iconify.design/material-symbols:folder-open-outline.svg?color=%23F59E0B" width="16" height="16" align="center" /> 2. โครงสร้าง Repository (Standard Course Layout)](#-2-โครงสร้าง-repository-standard-course-layout)
- [<img src="https://api.iconify.design/material-symbols:school-outline.svg?color=%230284C7" width="16" height="16" align="center" /> 3. เนื้อหาและการบรรยาย (Lectures & Slides)](#-3-เนื้อหาและการบรรยาย-lectures--slides)
- [<img src="https://api.iconify.design/material-symbols:terminal-outline.svg?color=%2310B981" width="16" height="16" align="center" /> 4. แบบฝึกหัดและการทดลองภาคปฏิบัติ (Labs & Assignments)](#-4-แบบฝึกหัดและการทดลองภาคปฏิบัติ-labs--assignments)
- [<img src="https://api.iconify.design/material-symbols:trophy-outline.svg?color=%23F59E0B" width="16" height="16" align="center" /> 5. โครงงานประจำรายวิชา (Course Projects)](#-5-โครงงานประจำรายวิชา-course-projects)
- [<img src="https://api.iconify.design/material-symbols:edit-note-outline.svg?color=%238B5CF6" width="16" height="16" align="center" /> 6. สรุปทบทวนและเตรียมสอบ (Exams Review)](#-6-สรุปทบทวนและเตรียมสอบ-exams-review)
- [<img src="https://api.iconify.design/material-symbols:verified-user-outline.svg?color=%23EF4444" width="16" height="16" align="center" /> 7. จริยธรรมทางวิชาการ (Academic Integrity Notice)](#-7-จริยธรรมทางวิชาการ-academic-integrity-notice)
- [<img src="https://api.iconify.design/material-symbols:person-outline.svg?color=%2306B6D4" width="16" height="16" align="center" /> 8. ผู้จัดทำ (Author)](#-8-ผู้จัดทำ-author)

---

## <img src="https://api.iconify.design/material-symbols:menu-book-outline.svg?color=%230284C7" width="22" height="22" align="center" /> 1. ข้อมูลรายวิชาเบื้องต้น (Course Information)

- **รหัสวิชา:** `JN60101`
- **ชื่อวิชาภาษาอังกฤษ:** Japanese for Communication 1
- **ชื่อวิชาภาษาไทย:** ภาษาญี่ปุ่นเพื่อการสื่อสาร 1
- **หน่วยกิต:** 3 หน่วยกิต
- **กลุ่มเรียน:** 1.2-1
- **ภาคการศึกษา / ปีการศึกษา:** ภาคเรียนที่ 1 / ปีการศึกษา 2569 (2026)
- **ผลการเรียนที่ได้รับ (Grade):** **กำลังศึกษา (In Progress / Enrolled - Term 1/2569)**
- **อาจารย์ผู้สอน (Instructor):** อาจารย์ ดร.เอกนรินทร์ จิรชีวีวงศ์
- **รูปแบบการเรียน:** Onsite 100% (วันพุธ)
- **ติดตามกำหนดส่งงานและคะแนนสะสม:** [Deadline Tracker](Deadline-Tracker.md)

### 📊 เกณฑ์การประเมินผลการเรียนรู้
| สัดส่วนคะแนน | รายละเอียดการประเมิน |
| :---: | :--- |
| **5%** | การเข้าชั้นเรียนและการมีส่วนร่วม (ตรงต่อเวลา ไม่ขาดเรียน) |
| **45%** | การทดสอบย่อย 3 ครั้ง (ครั้งละ 15 คะแนน, สอบพูดรายบุคคล 3 นาที) |
| **25%** | งานที่ได้รับมอบหมาย — งานเดี่ยวอธิบายไวยากรณ์ (10%) + งานกลุ่ม Role Play (15%) |
| **25%** | การทดสอบความรู้รวบยอดปลายภาค (Final Exam — การฟังและการพูด) |

---

## <img src="https://api.iconify.design/material-symbols:folder-open-outline.svg?color=%23F59E0B" width="22" height="22" align="center" /> 2. โครงสร้าง Repository (Standard Course Layout)

```text
Course-JN60101-Japanese-for-Communication-1-2026/
├── 00_Templates/               # Template โน้ตสรุปและคู่มือ Markdown/Obsidian
├── 01_Lectures/                # เอกสารการสอน สไลด์ และเลกเชอร์สรุปเข้มข้น
│   ├── 01_Docs/               # ประมวลรายวิชา และเอกสารอ้างอิง (Ignored in Git)
│   ├── 02_Teaching_Slides/    # สไลด์บรรยายประจำบท (Ignored in Git)
│   ├── Chapter1/              # บทที่ 1: การแนะนำตัวและระบบตัวอักษร
│   ├── Chapter2/              # บทที่ 2: การแลกนามบัตรและสิ่งของรอบตัว
│   ├── Chapter3/              # บทที่ 3: การบอกเวลาและสถานที่
│   ├── Chapter4/              # บทที่ 4: การซื้อของ 1 (ตัวเลขและราคา)
│   ├── Chapter5/              # บทที่ 5: การซื้อของ 2 (คุณลักษณะและลักษณนาม)
│   └── Chapter6/              # บทที่ 6: การยืนยันกำหนดการและการเดินทาง
├── 02_Labs_Assignments/       # ใบงาน เตรียมสอบย่อย และสคริปต์การแสดง
│   ├── Assignment1-Grammar-Video.md
│   ├── Assignment2-Roleplay.md
│   ├── Quiz1-Prep.md
│   ├── Quiz2-Prep.md
│   ├── Quiz3-Prep.md
│   └── Roleplay-Group-Sheet-Spec.md
├── 03_Projects/                # โครงงานบทสนทนาและสื่อการเรียนรู้ภาษาญี่ปุ่น
│   └── README.md
├── 04_Exams_Review/            # สรุปทบทวนเนื้อหาและเตรียมสอบปลายภาค
│   └── Final-Exam-Review.md
├── Deadline-Tracker.md         # เช็คลิสต์ติดตามภารกิจและกำหนดส่งงาน
└── README.md                   # สารบัญหลักและภาพรวมรายวิชา
```

> **หมายเหตุ:** โครงสร้างนี้รองรับการเปิดอ่านบน GitHub และเปิดเป็น **Obsidian Vault** โดยสมบูรณ์ (รองรับ Furigana, Callouts, Markdown Tables)

---

## <img src="https://api.iconify.design/material-symbols:school-outline.svg?color=%230284C7" width="22" height="22" align="center" /> 3. เนื้อหาและการบรรยาย (Lectures & Slides)

| บทเรียน | หัวข้อการเรียนรู้ (Topics) | โน้ตสรุปบทเรียน (MOC & Notes) | สไลด์บรรยาย |
| :---: | :--- | :--- | :---: |
| **Chapter 1** | **Self-Introduction (自己紹介)**<br>• ระบบตัวอักษร Hiragana / Katakana / Kanji และกฎการออกเสียง<br>• โครงสร้างไวยากรณ์ `N1 wa N2 desu / dewa arimasen`<br>• คำสรรพนาม การบอกสัญชาติ และอาชีพ | [Chapter 1 MOC](01_Lectures/Chapter1/Chapter1-MOC.md)<br>• [Writing System](01_Lectures/Chapter1/Japanese-Writing-System.md)<br>• [Pronunciation](01_Lectures/Chapter1/Pronunciation-Rules.md)<br>• [Sentence Structure](01_Lectures/Chapter1/Sentence-Structure-Wa-Desu.md)<br>• [Dialogue Practice](01_Lectures/Chapter1/Self-Introduction-Dialogue.md) | [สไลด์](01_Lectures/02_Teaching_Slides/) |
| **Chapter 2** | **Exchanging Business Cards (名刺交換)**<br>• คำสรรพนามชี้สิ่งของ `kore / sore / are` และคำถาม `dore`<br>• คำช่วยแสดงความเป็นเจ้าของ `N1 no N2`<br>• ตัวเลข 1-10 และบทสนทนาการแลกนามบัตรในธุรกิจ | [Chapter 2 MOC](01_Lectures/Chapter2/Chapter2-MOC.md)<br>• [Demonstratives](01_Lectures/Chapter2/Demonstratives-Kore-Sore-Are.md)<br>• [Possessive No](01_Lectures/Chapter2/Possessive-No-Particle.md)<br>• [Business Greetings](01_Lectures/Chapter2/Greetings-Self-Intro-Dialogue.md) | [สไลด์](01_Lectures/02_Teaching_Slides/) |
| **Chapter 3** | **Talking About Time (時間と場所)**<br>• คำชี้สถานที่ `koko / soko / asoko` และคำถาม `doko`<br>• การบอกเวลา ชั่วโมง นาที (`~ji / ~fun`) และตัวเลข 1-100<br>• คำช่วยแสดงช่วงเวลาและสถานที่ `kara ... made` | [Chapter 3 MOC](01_Lectures/Chapter3/Chapter3-MOC.md)<br>• [Places Vocabulary](01_Lectures/Chapter3/Places-Vocabulary.md)<br>• [Telling Time](01_Lectures/Chapter3/Telling-Time-Hours-Minutes.md)<br>• [Kara Made Grammar](01_Lectures/Chapter3/Kara-Made-Grammar.md) | [สไลด์](01_Lectures/02_Teaching_Slides/) |
| **Chapter 4** | **Buying Things 1 (買い物 1)**<br>• คำขยายคำนาม `kono / sono / ano + N`<br>• ระบบตัวเลขหลักร้อยถึงล้าน (`hyaku, sen, man`) และการถามราคา<br>• สำนวนการสั่งซื้อ `~ o kudasai` | [Chapter 4 MOC](01_Lectures/Chapter4/Chapter4-MOC.md)<br>• [Demonstratives Modifier](01_Lectures/Chapter4/Kore-Sore-Are-Kono-Sono-Ano.md)<br>• [Numbers Hyaku to Oku](01_Lectures/Chapter4/Numbers-Hyaku-Sen.md)<br>• [Shopping Expressions](01_Lectures/Chapter4/Shopping-Requests-Kudasai.md) | [สไลด์](01_Lectures/02_Teaching_Slides/) |
| **Chapter 5** | **Buying Things 2 (買い物 2)**<br>• คำคุณศัพท์แสดงขนาด สี และรูปลักษณ์<br>• ลักษณนามภาษาญี่ปุ่น (อัน ชิ้น แผ่น เล่ม)<br>• การระบุแหล่งกำเนิดและประเทศผู้ผลิต `[ประเทศ] no [สินค้า]` | [Chapter 5 MOC](01_Lectures/Chapter5/Chapter5-MOC.md)<br>• [Adjectives Size & Color](01_Lectures/Chapter5/Adjectives-Size-Color.md)<br>• [Japanese Counters](01_Lectures/Chapter5/Counters-Japanese.md)<br>• [Country Origin](01_Lectures/Chapter5/Countries-Made-In.md) | [สไลด์](01_Lectures/02_Teaching_Slides/) |
| **Chapter 6** | **Confirming Schedule (スケジュールの確認)**<br>• คำบอกเวลาสัมพัทธ์ (วันนี้ พรุ่งนี้ สัปดาห์หน้า เดือนหน้า)<br>• กริยาการเคลื่อนที่ `ikimasu / kimasu / kaerimasu`<br>• การระบุวันในสัปดาห์ วันที่ และเดือน | [Chapter 6 MOC](01_Lectures/Chapter6/Chapter6-MOC.md)<br>• [Movement Verbs](01_Lectures/Chapter6/Ikimasu-Kimasu-Kaerimasu-Grammar.md)<br>• [Time Expressions](01_Lectures/Chapter6/Time-Expressions-Kinou-Ashita.md)<br>• [Date Expressions](01_Lectures/Chapter6/Date-Time-Expressions.md) | [สไลด์](01_Lectures/02_Teaching_Slides/) |

---

## <img src="https://api.iconify.design/material-symbols:terminal-outline.svg?color=%2310B981" width="22" height="22" align="center" /> 4. แบบฝึกหัดและการทดลองภาคปฏิบัติ (Labs & Assignments)

| ลำดับงาน | หัวข้องาน / การประเมิน | รายละเอียดและเกณฑ์คะแนน | เอกสารสคริปต์/แนวเตรียม |
| :---: | :--- | :--- | :---: |
| **Quiz 1** | ทดสอบย่อยครั้งที่ 1 (บทที่ 1-2) | การแนะนำตัว, การแลกนามบัตร, และคำศัพท์สิ่งของ (15 คะแนน) | [แนวเตรียม Quiz 1](02_Labs_Assignments/Quiz1-Prep.md) |
| **Quiz 2** | ทดสอบย่อยครั้งที่ 2 (บทที่ 3-4) | การบอกเวลา สถานที่ และการซื้อของพื้นฐาน (15 คะแนน) | [แนวเตรียม Quiz 2](02_Labs_Assignments/Quiz2-Prep.md) |
| **Quiz 3** | ทดสอบย่อยครั้งที่ 3 (บทที่ 5-6) | คำคุณศัพท์ ลักษณนาม และการนัดหมายกำหนดการ (15 คะแนน) | [แนวเตรียม Quiz 3](02_Labs_Assignments/Quiz3-Prep.md) |
| **Assign 1** | การอธิบายไวยากรณ์ (Grammar Video) | งานเดี่ยว จัดทำวิดีโออธิบายโครงสร้างไวยากรณ์ภาษาญี่ปุ่น (10 คะแนน) | [รายละเอียดงาน 1](02_Labs_Assignments/Assignment1-Grammar-Video.md) |
| **Assign 2** | การแสดงบทบาทสมมติ (Role Play) | งานกลุ่ม จัดทำบทสนทนาสถานการณ์ธุรกิจและการแสดงจริง (15 คะแนน) | [สคริปต์และข้อกำหนด](02_Labs_Assignments/Assignment2-Roleplay.md)<br>[Group Sheet Spec](02_Labs_Assignments/Roleplay-Group-Sheet-Spec.md) |

---

## <img src="https://api.iconify.design/material-symbols:trophy-outline.svg?color=%23F59E0B" width="22" height="22" align="center" /> 5. โครงงานประจำรายวิชา (Course Projects)

> โครงงานการสื่อสารภาษาญี่ปุ่นและสื่อมัลติมีเดีย (เก็บอยู่ในโฟลเดอร์ `03_Projects/`)

### 🎌 Business Japanese Dialogue & Interactive Communication Project
- **บทบาทและหน้าที่:** โครงงานจัดทำสื่อและสถานการณ์จำลองเพื่อพัฒนาทักษะการฟัง พูด และการสื่อสารเชิงธุรกิจในบริบทวัฒนธรรมญี่ปุ่น
- **เทคโนโลยีและเครื่องมือ:** `Audio/Video Production, Markdown Scripting, Interactive Vocabulary Flashcards`
- **ซอร์สโค้ดและสื่อ:** [โฟลเดอร์โครงงาน](03_Projects/)

---

## <img src="https://api.iconify.design/material-symbols:edit-note-outline.svg?color=%238B5CF6" width="22" height="22" align="center" /> 6. สรุปทบทวนและเตรียมสอบ (Exams Review)

- [x] **การทดสอบความรู้รวบยอดปลายภาค (Final Comprehensive Exam):** [สรุปทบทวนการฟังและการพูดฉบับเต็ม](04_Exams_Review/Final-Exam-Review.md)

---

## <img src="https://api.iconify.design/material-symbols:verified-user-outline.svg?color=%23EF4444" width="22" height="22" align="center" /> 7. จริยธรรมทางวิชาการ (Academic Integrity Notice)

> [!NOTE]  
> คลังนี้จัดทำขึ้นเพื่อเป็น **บันทึกการเรียนรู้ส่วนบุคคล (Personal Learning Archive)** และนำเสนอพัฒนาการทางวิชาการ (Academic Portfolio) เท่านั้น  
> ไม่อนุญาตให้นำโค้ด การบ้าน หรือรายงานไปคัดลอก (Plagiarism) เพื่อส่งงานในรายวิชาโดยไม่ได้รับอนุญาตตามระเบียบของสถาบันฯ

---

## <img src="https://api.iconify.design/material-symbols:person-outline.svg?color=%2306B6D4" width="22" height="22" align="center" /> 8. ผู้จัดทำ (Author)

**Phuriphat Hemakul (PhuriphatTyPeZ3r0)**
- <img src="https://api.iconify.design/material-symbols:school-outline.svg?color=%230284C7" width="16" height="16" align="center" /> นักศึกษา สาขาวิศวกรรมคอมพิวเตอร์และปัญญาประดิษฐ์ (CAI)
- <img src="https://api.iconify.design/material-symbols:apartment-rounded.svg?color=%230284C7" width="16" height="16" align="center" /> สถาบันการจัดการปัญญาภิวัฒน์ (PIM)
- <img src="https://api.iconify.design/simple-icons:github.svg?color=%23181717" width="16" height="16" align="center" /> GitHub: [@PhuriphatTyPeZ3r0](https://github.com/PhuriphatTyPeZ3r0)
- <img src="https://api.iconify.design/material-symbols:language.svg?color=%233B82F6" width="16" height="16" align="center" /> Portfolio: [resume-phuriphat-hemakul.vercel.app](https://resume-phuriphat-hemakul.vercel.app)
