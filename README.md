# JN60101 Japanese for Communication 1 (ภาษาญี่ปุ่นเพื่อการสื่อสาร 1)

คลังสรุปเนื้อหา คำศัพท์ ไวยากรณ์ และแบบฝึกหัดรายวิชา **JN60101 ภาษาญี่ปุ่นเพื่อการสื่อสาร 1**
สถาบันการจัดการปัญญาภิวัฒน์ (PIM) — ภาคการศึกษา 1/2569, กลุ่มเรียน 1.2-1

---

## 📌 ข้อมูลรายวิชาเบื้องต้น

- **อาจารย์ผู้สอน:** อาจารย์ ดร.เอกนรินทร์ จิรชีวีวงศ์ (กลุ่มวิชาภาษาเพื่อการสื่อสาร สำนักการศึกษาทั่วไป)
- **รูปแบบการเรียน:** Onsite 100%
- **ช่องทางหลัก:** LINE กลุ่ม (ติดต่อสื่อสาร) และ PIM e-Learning (ส่งงานทุกชิ้น)
- **วัตถุประสงค์:** (1) ฟังภาษาญี่ปุ่นเบื้องต้นออก (2) สื่อสารภาษาญี่ปุ่นพื้นฐานได้ (3) มีความสุขในการเรียน

### 🎯 เกณฑ์การประเมินผล (อิงเกณฑ์)

| สัดส่วนคะแนน | รายละเอียด |
|---|---|
| **5%** | การเข้าชั้นเรียนและการมีส่วนร่วม (สาย 4 ครั้ง = ขาด 1 ครั้ง) |
| **45%** | ทดสอบย่อย 3 ครั้ง (ครั้งละ 15 คะแนน, สอบพูดคนละ 3 นาที) |
| **25%** | งานที่ได้รับมอบหมาย — งานเดี่ยวอธิบายไวยากรณ์ 10 คะแนน + งานกลุ่ม Role Play 15 คะแนน |
| **25%** | การทดสอบความรู้รวบยอด (สอบปลายภาค — การฟังและการพูด) |

**เกณฑ์เกรด:** 80-100=A · 75-79=B+ · 70-74=B · 65-69=C+ · 60-64=C · 55-59=D+ · 50-54=D · 0-49=F

### 🗓️ ตารางเรียนกลุ่ม 1.2-1

| สัปดาห์ | วัน | เนื้อหา |
|---|---|---|
| 1 | พุธ 9 กย 2569 | แนะนำรายวิชา + บทที่ 1 |
| 2 | พุธ 16 กย 2569 | บทที่ 1-2 |
| 3 | พุธ 23 กย 2569 | **สอบย่อยครั้งที่ 1** (บทที่ 1-2) |
| 4 | พุธ 30 กย 2569 | บทที่ 3 |
| 5 | พุธ 7 ตค 2569 | บทที่ 4 |
| 6 | พุธ 14 ตค 2569 | **สอบย่อยครั้งที่ 2** (บทที่ 3-4) |
| 7 | พุธ 21 ตค 2569 | บทที่ 5 |
| 8 | พุธ 28 ตค 2569 | บทที่ 6 |
| 9 | พุธ 4 พย 2569 | **สอบย่อยครั้งที่ 3** (บทที่ 5-6) |
| 10 | พุธ 11 พย 2569 | **สอบการฟังและการพูดปลายภาค** |

**งานทุกชิ้นส่งในกล่องส่งงานใน e-learning ภายในวันศุกร์ที่ 13 พฤศจิกายน 2569 เวลา 17.00 น.** — ดูรายละเอียดฉบับเต็มที่ [Deadline Tracker](Deadline-Tracker.md)

---

## 🗂️ โครงสร้าง Repository (Project Structure)

```text
04_JN60101_Japanese-1/
├── 00_Templates/              # Template โน้ตและคู่มือ format
├── 01_Lectures/
│   ├── 01_Teaching_Slides/    # สไลด์ประกอบการสอนต้นฉบับ (Ignored in Git)
│   └── Chapter1-6/            # โน้ตสรุปเนื้อหาแต่ละบท (Markdown / Obsidian)
├── 02_Labs_Assignments/       # ใบงาน, เตรียมสอบย่อย, งานที่ได้รับมอบหมาย
├── 03_Projects/               # (ไม่ใช้งานในวิชานี้)
├── 04_Exams_Review/           # สรุปทบทวนก่อนสอบปลายภาค
├── Deadline-Tracker.md        # ติดตามกำหนดส่งงานและคะแนนสะสม
└── README.md                  # เอกสารแนะนำและสารบัญหลัก
```

---

## 📖 สารบัญสรุปเนื้อหา (Chapter Index)

- [Chapter 1 — Self-Introduction](01_Lectures/Chapter1/Chapter1-MOC.md) — ระบบตัวอักษร, กฎการออกเสียง, N wa N desu, สรรพนาม, การแนะนำตัว
- [Chapter 2 — Exchanging Business Cards](01_Lectures/Chapter2/Chapter2-MOC.md) — คำศัพท์สิ่งของ, kore/sore/are, N1 no N2, การแลกนามบัตร
- [Chapter 3 — Talking About Time](01_Lectures/Chapter3/Chapter3-MOC.md) — こそあど, คำศัพท์สถานที่, ตัวเลข 1-100, การบอกเวลา, kara...made
- [Chapter 4 — Buying Things 1](01_Lectures/Chapter4/Chapter4-MOC.md) — คำศัพท์ซื้อของ, ตัวเลขหลักร้อย-พัน-หมื่น-แสน-ล้าน, ~kudasai
- [Chapter 5 — Buying Things 2](01_Lectures/Chapter5/Chapter5-MOC.md) — คำคุณศัพท์ขนาด/สี, ลักษณนาม, ประเทศผู้ผลิต, บทสนทนาซื้อของ
- [Chapter 6 — Confirming Schedule](01_Lectures/Chapter6/Chapter6-MOC.md) — คำบอกเวลาสัมพัทธ์, ikimasu/kimasu/kaerimasu, การบอกวันเดือนปี

## 📝 งานและการสอบ (Assignments & Exams)

- [Quiz 1 Prep](02_Labs_Assignments/Quiz1-Prep.md) (บทที่ 1-2) · [Quiz 2 Prep](02_Labs_Assignments/Quiz2-Prep.md) (บทที่ 3-4) · [Quiz 3 Prep](02_Labs_Assignments/Quiz3-Prep.md) (บทที่ 5-6)
- [Assignment 1 — Grammar Video](02_Labs_Assignments/Assignment1-Grammar-Video.md) (งานเดี่ยว, 10 คะแนน)
- [Assignment 2 — Role Play](02_Labs_Assignments/Assignment2-Roleplay.md) (งานกลุ่ม, 15 คะแนน) · [Group Sheet Spec for Antigravity](02_Labs_Assignments/Roleplay-Group-Sheet-Spec.md)
- [Final Exam Review](04_Exams_Review/Final-Exam-Review.md) (สอบปลายภาค, 25 คะแนน)

---

## 💡 วิธีการใช้งาน (How to Use)

- **เปิดอ่านผ่าน GitHub:** สามารถคลิกลิงก์ Markdown ด้านบนเพื่ออ่านเนื้อหาผ่าน GitHub ได้ทันที
- **เปิดผ่าน Obsidian:** สามารถเปิดโฟลเดอร์นี้เป็น Obsidian Vault ได้ทันที รองรับ Wikilinks, Callouts (`> [!info]`), และ Mermaid Diagrams
